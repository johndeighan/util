...
# --- compile all library files - depends on my directory structure!
deno run -A @danielx/civet --config c:/Users/johnd/civetconfig.json --inline-map -o .ts -c src/lib/*.lib.civet
deno check src/lib/*.lib.ts

# --- compile the file
deno run -A @danielx/civet --config c:/Users/johnd/civetconfig.json -o .ts -c src/exe/compile-libs.exe.civet
deno check src/exe/compile-libs.exe.ts

# --- Install as an exe
deno compile src/exe/compile-libs.exe.ts
